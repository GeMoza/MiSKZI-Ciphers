...
UI_STRINGS: Final[dict[str, dict[str, str]]] = {
    "ru": {
        ...
        "Load free_text": "Загрузить свободный текст",
        "Loaded variant": "Вариант загружен",
        "Loaded free_text": "Свободный текст загружен",
        "Loaded key example": "Пример ключа подгружен",
        ...
        "Save meta": "Сохранить заметки/свободный текст",
        "Saved meta": "Метаданные сохранены",
        ...
        "Raw key example": "Пример ключа (JSON)",
        ...
        "raw key example JSON must be object": "Пример ключа должен быть JSON-объектом",
        ...
        "Apply key example": "Подгрузить пример ключа",
    ...
    "en": {
        ...
        "Load free_text": "Load free text",
        "Loaded variant": "Loaded variant",
        "Loaded free_text": "Free text loaded",
        "Loaded key example": "Key example loaded",
        ...
        "Save meta": "Save notes/free text",
        "Saved meta": "Meta saved",
        "Validation errors": "Validation errors",
        "No params": "No params",
        "No variants": "No variants",
        "JSON error": "JSON error",
        "Raw JSON key must be an object": "Raw JSON key must be an object",
        "Optional": "optional",
        "Encrypted": "Encrypted",
        "Decrypted action": "Decrypted",
        "Key JSON object": "key (JSON object)",
        "Raw key example": "Raw key example (JSON)",
        "Expected optional": "expected (optional)",
        "key JSON must be object": "key JSON must be object",
        "key JSON error": "key JSON error",
        "raw key example JSON must be object": "Raw key example must be a JSON object",
        "Cannot save key JSON object": "Cannot save: key must be a valid JSON object",
        "Cannot run key JSON object": "Cannot run: key must be a valid JSON object",
        "Saved variants": "variants.json saved",
        "Deleted variant": "Variant deleted",
        "Result": "Result",
        "expected match": "expected match",
        "expected mismatch": "expected mismatch",
        "free_text file": "free_text.txt",
        "Saved free_text": "free_text.txt saved",
        ...
        "Text": "Text",
        "Key input": "Key input",
        "Apply JSON to Form": "Apply JSON to Form",
        "Apply key example": "Apply key example",
        "Sync to Raw JSON": "Sync to Raw JSON",
...
CIPHER_LABELS: Final[dict[str, dict[str, str]]] = {
    "ru": {
        ...
        "elgamal": "Криптосистема Эль-Гамаля",
        ...
        "rsa": "Криптографический алгоритм RSA",
...
PARAM_LABELS: Final[dict[str, dict[str, str]]] = {
    "ru": {
        ...
        "elgamal.p": "Число p",
        "elgamal.g": "Число g",
        "elgamal.x": "Закрытый ключ x",
        "elgamal.k": "Сессионный ключ k",
        "gronsfeld.digits": "Цифровой ключ",
        "hill.matrix": "Матрица ключа",
        "hill.pad_char": "Символ дополнения",
        "polybius.method": "Метод",
        "ramsey.keyword": "Ключевое слово",
        "ramsey.keyword_table": "Ключевое слово таблицы",
        "ramsey.anagram": "Анаграмма",
        "ramsey.group_size": "Размер группы",
        "richelieu.key": "Ключ (строка перестановок)",
        "richelieu.permutations": "Перестановки",
        "rsa.p": "Число p",
        "rsa.q": "Число q",
        "rsa.e": "Открытый показатель e",
        "rubik_2x2.moves": "Последовательность ходов",
...
PARAM_HELPS: Final[dict[str, dict[str, str]]] = {
    "ru": {
        ...
        "elgamal.p": "Простое число p, большее сообщения M.",
        "elgamal.g": "Первообразный корень по модулю p.",
        "elgamal.x": "Закрытый ключ x из ПЗ-06.",
        "elgamal.k": "Сессионный ключ k, взаимно простой с p-1.",
        ...
        "ramsey.keyword": "Латинское ключевое слово для построения таблицы.",
        "ramsey.keyword_table": "Фиксированное слово SUBWAY из таблицы 29 методички.",
        "ramsey.anagram": "Латинский ключ перестановки без повторов.",
        "ramsey.group_size": "Размер группировки цифр в выводе.",
        "richelieu.key": "Нотация вида (4213)(51243).",
        "richelieu.permutations": "Список перестановок, например [[4,2,1,3],[5,1,2,4,3]].",
        "rsa.p": "Первое простое число для вычисления модуля n = p*q.",
        "rsa.q": "Второе простое число для вычисления модуля n = p*q.",
        "rsa.e": "Открытый показатель, взаимно простой с (p-1)*(q-1).",
        "rubik_2x2.moves": "JSON-список ходов по развёртке ПЗ-05.",
