...
В `docs/pdf/` можно хранить локальные копии методичек и вспомогательные PDF для работы над проектом. Эта папка считается справочной: структура каталога сохраняется в репозитории, но сами PDF-файлы не должны коммититься. Для этого в `.gitignore` есть отдельные правила.
