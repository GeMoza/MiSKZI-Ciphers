...
| `kuznechik` | yes | yes | yes | yes | yes | В доступной ПЗ-7 тема только упоминается в конце без алгоритма и вариантов; текущая реализация имеет статус `methodics-theory-only` + project control set для одного 128-битного hex-блока. |
